...
plugin's own suite gets these tests too. Measured from an installed copy: 11
failures, none of them a real defect, all of them "this is not the source repo".
...
def pytest_collection_modifyitems(config, items):
    ...
    skip = pytest.mark.skip(
        reason=(
            "consistency-checks guards the repo that develops the harness; this "
            "is an installed copy, so these assertions do not apply here"
        )
    )
    for item in items:
