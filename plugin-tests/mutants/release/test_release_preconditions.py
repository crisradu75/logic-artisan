...
MUTANTS = [
    ...
    (
        # An exclusion creeping back in. Whatever file it names, an uncommitted
        # edit to it rides into the tag and a conflict in it passes unseen.
        "an exclusion lets an uncommitted file into the release",
        SKILL,
        "git status --porcelain" + _NL + "git fetch origin",
        "git status --porcelain -- :/ ':(exclude)CLAUDE.md'" + _NL + "git fetch origin",
        TARGETS,
    ),
    (
        "a `.` pathspec narrows the clean-tree check to the current directory",
        SKILL,
        "git status --porcelain" + _NL + "git fetch origin",
        "git status --porcelain -- ." + _NL + "git fetch origin",
        TARGETS,
