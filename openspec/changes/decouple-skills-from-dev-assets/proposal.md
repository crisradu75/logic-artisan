...
This is change (a) of three from `cla.io/decisions/ship-only-consumer-usable-assets-2026-08-22.md`.
It is deliberately all in-place content edits and **no file moves**, so each rewrite lands against a
file that has not moved and the diff shows what changed rather than that it moved. Git renders a
rewritten-and-moved file as delete+add, which would make the highest-risk edits here the least
reviewable if bundled with the ~70 renames that change (b) performs.
...
- **Promote the two downstream-live conformance guards to skill scripts.** Each becomes a program
  ...
  the new modules; change (b) relocates those tests.
...
- **Rename both colliding `aggregate.py` modules** to distinct names, with their `test_aggregate.py`
  counterparts, and update every SKILL.md line, import, and hardcoded path list that names them.
  The 12-scope pytest split rests on exactly these two collisions; renaming them is the precondition
  for change (b) collapsing the dev tree to one scope.
...
- **Fix the doc lines this change falsifies** — `skills/sync-context/SKILL.md:151` and
  ...
  deferred to change (c), `guard-shipped-asset-boundary`.

Not breaking for consumers: no skill is removed, no invocation a consumer relies on disappears —
`mutate.py` itself still exists in the tree after this change (change (b) removes it).
...
- `cla-plugin`: three requirements change.
  ...
  - **Repo-state resolution seam** — names "each retro loop's `aggregate.py`", which the rename
    ...
    `<git rev-parse --show-toplevel>/cla.io/retro`) is unchanged; only the module names it cites move.
  - Plus one **new requirement** added to the same spec, **Review-fix evidence gate**, pinning the
    prose form of the mutation gate so the rule survives `mutate.py`'s removal in change (b).
...
- `skills/lite-pr/SKILL.md` (step 2b, line 129)
- `skills/spec-to-pr/references/revise.md` (step 2b, line 94)
...
`spec-to-pr-retro/`. `run_tests.py` remains the gate for this change (change (b) deletes it).

**Out of scope**, and owned by the two follow-on changes: moving anything to `<repo>/plugin-tests/`;
deleting `run_tests.py` or `mutate.py`; moving the `release` skill; the release-time tree scan; and
the layout/scope-count reconciliation across `CLAUDE.md`, `DEVELOPER-GUIDE.md`, the plugin
`README.md`, root `.gitattributes`, and `TODO.md`.
