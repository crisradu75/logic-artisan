...
**`scrollIntoView` is cross-boundary geometry that `contentRect()` does NOT cover**, and it has a second caller. `focusBlock()` (`:1029`-`:1035`) resolves a block and calls `.scrollIntoView()` on it; it is wired to the drawer's "go to passage" button (`:1892`-`:1895`) and is the same mechanism the rail reuses. Because it is not a `getBoundingClientRect()` call, routing the rect sites through a helper leaves it untouched. Whether `scrollIntoView` on a node inside a content-sized same-origin frame scrolls the OUTER page is not something this design should assert from reasoning — **it is a browser behaviour, and it is settled in the Playwright suite before the implementation depends on it.** If it does not propagate, both the rail and "go to passage" silently fail to move the page, which is the same invisible class of defect as the margin-note offset.
