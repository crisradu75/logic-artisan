# review-change — project context overlay

<!--
Project-specific overlay for the `review-change` cla skill. This file is repo-local
...
## Real-data/scale failure classes
<!-- Consumed by checklist.md check 0j. This repo's concrete, already-bitten limits
     when parsing/bulk-loading a real data artifact: the runtime's string/buffer size
     cap, the datastore's transaction/lock ceiling and whether it accumulates across
     statements, the real files' encoding/delimiter conventions — with the actual
     numbers and the workaround that worked. Generic 0j supplies the two questions
     (shape, scale); this section supplies the answers for this stack. -->
