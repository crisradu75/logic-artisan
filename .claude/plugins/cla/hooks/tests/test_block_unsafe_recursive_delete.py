...
def test_detects_windows_backslash_path():
    ...
    paths = hook._extract_target_paths(r"rm -rf C:\Code\agentic-air\.claude\worktrees\x")
    assert r"C:\Code\agentic-air\.claude\worktrees\x" in paths
...
def test_worktree_path_detected_regardless_of_separator():
    assert hook._is_worktree_path(Path("C:/Code/agentic-air/.claude/worktrees/some-change"))
    assert hook._is_worktree_path(Path(r"C:\Code\agentic-air\.claude\worktrees\some-change"))
