"""Append one JSON line per /spec-to-pr run to the project's runs log.

Called by /spec-to-pr's Handoff phase AFTER the terminal report is printed.
Reads a JSON object from stdin (the run record assembled by the orchestrator
from its in-context phase outcomes) and appends it as a single line to:

    <repo-root>/cla.io/retro/spec-to-pr-runs.jsonl
...
var (absolute path). The companion `.gitattributes` sets `merge=union` on
`cla.io/retro/*.jsonl` so concurrent appends from two machines auto-resolve by
keeping both lines.
...
are atomic, so concurrent /spec-to-pr runs from parallel sessions cannot
interleave bytes within a line. Windows offers the same effective guarantee
for small writes to local files. This avoids the read-modify-rewrite pattern,
which is both O(N) per append and silently race-unsafe (two readers see the
same N, both rewrite, one append is lost).

Exit codes:
  0 = appended successfully
  1 = JSON parse failure or write failure (stderr names which)
...
run. The orchestrator's Handoff step ignores non-zero exits and continues.
"""
...
def _runs_dir() -> Path:
    ...
    machine-local `~/.claude/projects/<hash>/`. CLAUDE_RETRO_DIR (absolute path)
    overrides it — used by tests and non-standard layouts.

    Raises on a non-absolute override or an unresolvable repo root rather than
    guessing a path: the consumer (`spec-to-pr-retro/scripts/aggregate.py`)
