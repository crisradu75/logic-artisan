...
4. **Post-check:** `openspec status --change <name> --json` returns `isComplete: true` AND every `- [ ]` in `tasks.md` is now `- [x]`. ✓ on both true. ⚠ otherwise; capture the unfinished tasks for the Handoff Issues section.
   ...
   **Both of those read the checkbox glyph, never the claim under it — so neither can catch a task that is ticked and untrue.** Measured in one chain: three ticked tasks asserted things that were false, including a mutation test that could not have failed anything, and every mechanical check passed all three. Two additions close it, and both are cheap:

   - **A task whose text asserts a MEASUREMENT records the measured value inline.** A confirmed value, a count, a mutation-test result — the tick is not the evidence, the number is: `- [x] 6.4 … measured: og:image:width=1200 on /, ABSENT on /exemplu`. A delegate that must produce a number produces a real one; a delegate that need only tick a box ticks it. The briefs that demanded numbers are the ones that came back with them.
   - **Re-measure a 2–3 task sample of the measurement-bearing tasks yourself**, rather than trusting the ticks wholesale. Sampling, not exhaustive re-verification — that would cost as much as the implementation. Two of the three false ticks above sat in a sample this size.

   **The convention, which the checks above only enforce and do not teach: a task is `[ ]` until it is done, and the prose beneath it explains why it is still open.** Ticking a box and writing "NOT DONE — blocked on X" underneath is the shape that makes every count wrong, and it is honest work filed dishonestly. Handoff greps for exactly that (see below), but the fix is to not write it.
...
- **Ticked-but-not-done scan, BEFORE anything else in this phase.** For every `^- \[x\]` line in `tasks.md`, read its body up to the next task marker or heading and fail Handoff on a case-insensitive hit for `NOT DONE`, `not done`, `could not`, or `deferred`. One grep, no judgement about whether the reason was good — a ticked task carrying its own denial makes `isComplete`, the box count and this report all wrong at once, and reading the paragraph is otherwise the only thing that would catch it. Recipe: `references/handoff.md`.
