...
git worktree add <worktrees-dir>/<change> -b feature/<change> origin/<base-branch>
# then launch Claude in <worktrees-dir>/<change> and run: /cla:spec-to-pr <change>
```

**Branch off `origin/<base-branch>` explicitly, never bare `git worktree add ... -b feature/<change>`.** Omitting the base branches off whatever the primary clone's *local* `<base-branch>` ref happens to point to, which drifts stale the moment any other change in a chain squash-merges (squash rewrites history, so a local `<base-branch>` that hasn't been fetched/pulled since diverges from `origin/<base-branch>` even with zero local commits of its own). A branch cut from that stale base produces a PR `gh` reports as not-cleanly-mergeable — recoverable (a cherry-pick-onto-a-fresh-branch-plus-force-push repair; see `references/project-context.md` for a real recovery precedent), but avoidable for a one-line fix. `git fetch origin <base-branch>` first makes `origin/<base-branch>` current without touching the primary clone's checked-out branch, so this is safe even while another session holds the primary clone.
...
**Both tiers empty means one of three different things, and they are NOT interchangeable.** `--staged` reports which: `no-source-affecting-paths` → status `skip`, a genuine docs-/openspec-only change with nothing to gate. `no-check-scripts` → `skip`; the manifest defines none. **`no-package-json` → NOT a skip.** This repo has no root `package.json`, so the script can discover nothing here at all — treat it as **`warn`**, and run the gates named in `cla.io/project-facts.md` ("Dev / build / test commands") or pass `--test-cmd "<cmd>"`. Reporting that case as a skip announces a source change as docs-only and silently runs no correctness gate at all; the reason is plausible enough that nobody questions it, which is what makes it worse than a missing gate.
