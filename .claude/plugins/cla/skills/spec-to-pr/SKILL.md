...
It returns the hit list `path:line — symbol`. Surface every hit as an Important Review finding. This matters especially for this repo's own load-bearing conventions (see `cla.io/overlays/spec-to-pr.md`) where a stale doc reference can mask a broken key contract.
