...
**Read `references/revise.md` first** — the agent-selection table, the round-1 `Workflow` fan-out snippet + diff-embedding discipline, the round-≥2 mechanics, the commit recipe, and the full prose behind each invariant. Cap: `--pr-rounds N` (default `2`) — a ceiling, not a promise of two rounds: every Critical and Important is triaged in the round that surfaced it, so the exit gate, not the cap, ordinarily ends Revise. Load-bearing invariants (hold these even if the reference isn't reloaded):
...
- **Round ≥2 carries its own question, in each agent's prompt:** *"Does this fix introduce the defect it fixed, somewhere else? Enumerate every other instance of the resource or shape the fix concerns."* The **enumeration is the deliverable** — a verdict per instance, and `enumerated: no other instance` as a stated result rather than a skipped step. Re-asking round 1's question over round 1's fix diff mostly re-confirms the fix, and a sibling instance is never in that diff. This raises no cap and does not make a second round automatic; `references/revise.md` carries why, and the condition that would reverse it.
