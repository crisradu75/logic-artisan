...
**Inherited obligations — a required output field, only when `--inherits` is set.** Split `<inherits>` on `;` and emit one line per entry as the FIRST thing in the report, ahead of every other finding:

```
<token>: HONOURED | VIOLATED | NOT ADDRESSED — <evidence>
```

Settle each verdict with one command rather than by reading: `grep -rl "<token>" openspec/changes/<change-name>/`. **No match anywhere in the change directory → `NOT ADDRESSED`, always** — a change cannot have honoured a field none of its documents names, and this is the case the evidence says recurs. A match → read it and decide between `HONOURED` (the artifacts consume the field the way the obligation requires) and `VIOLATED` (they name it and get it wrong, or contradict it — an `## Impact` still saying "Migration: none", a Non-Goals section still calling the producing module read-only). **`VIOLATED` and `NOT ADDRESSED` are each a Critical finding**, handled by step 4's apply loop like any other, so the fix edits `proposal.md`/`design.md`/`tasks.md`/`specs/` to carry the obligation *before* Implement runs. This block is required output, not optional context: **done is countable — one verdict line per `;`-separated entry, and a missing line is a failed round, not a pass.** It is a required field precisely because the discipline it replaces was optional; asking a reviewer to answer for a named obligation is what caught this defect both times it was caught, while re-reading a carry list at one's own initiative caught it never.
