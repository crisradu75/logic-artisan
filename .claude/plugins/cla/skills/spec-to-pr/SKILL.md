...
Because every default-path message is a single line, **no scratch files are needed in the happy path** — trailers included. Use `git add -- <paths>` then `git commit -m "<subject>"`, adding a single second `-m` holding every `Measured-by:` line newline-separated when the change asserts a measurement (one `-m` per trailer splits them into separate paragraphs and stops them being a trailer block), and `gh pr create --body "<one-line body>"`. **The session-attribution lines — `Co-Authored-By:` and `Claude-Session:` — belong in that same `-m`.** They go directly under the last `Measured-by:` line, with no blank line between them. Git parses trailers from the last paragraph only, and that paragraph must be `key: value` lines throughout. One line without that shape drops every measurement out of the block. The recorded count silently becomes zero. A bare `Closes #199` reference line is the usual offender. The `gh pr create --body "..."` parser bug is triggered by *newline-followed-by-`#`*, not by long single lines without `#`, so a one-liner is safe.
