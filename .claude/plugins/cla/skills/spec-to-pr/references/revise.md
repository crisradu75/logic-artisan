...
2. **Triage every Critical and Important finding into one of three buckets:**
   ...
   **A rejection with no reason that resolves against the brief's own defect or fact rows is not a rejection.** Treat it as an unaddressed finding. The reason is what keeps `remedy-rejected` from becoming the cheapest exit from any hard finding, and re-deciding the remedy surfaces a bad rejection immediately.
   ...
   **Receiving what it returns** — read the per-finding list, not the overall status, and route each row:
   ...
   A contract firing here — a `done` row without defect-gone evidence, or a missing per-finding list — is recorded as a Handoff issue, the same as Implement's. Recovering inline is correct; absorbing it silently hides the delegation-reliability signal.
...
4. **Exit gate — TWO counts, and both must be zero to exit clean.** *Triaged* and *closed* are different predicates, and one counter cannot serve for both.
   ...
   If **both** are 0 → status `ok`, exit loop. If open > 0 with budget remaining → **re-enter the loop** and re-attempt those findings with re-decided remedies. Suggestions still recorded for the optional `docs:` commit.
