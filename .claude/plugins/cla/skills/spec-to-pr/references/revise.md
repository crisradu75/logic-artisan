...
**`code-reviewer` and `silent-failure-hunter` are never demoted — in ANY round** (the round-≥2 tier-down rule below exempts them) — a cheaper bug-hunter emits more phantom findings, whose triage cost exceeds the saving, and that economics is not round-scoped (see `${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/model-routing.md` rationale). And because `routing.revise_findings_by_tier` is keyed per AGENT (not per model tier — schema pinned in `${CLAUDE_PLUGIN_ROOT}/skills/_shared/references/run-log-schema.md`), a demoted bug-hunter's degraded accuracy lands under its own agent key, so a rising phantom rate shows up directly against `code-reviewer`/`silent-failure-hunter` in `/cla:spec-to-pr-retro` — exactly the signal that polices this rule.
