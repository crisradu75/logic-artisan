...
2. **Triage every Critical and Important finding into one of three buckets:**
   ...
   **Closing by disproof requires the orchestrator to re-run the defect check — the delegate's word is not the evidence.** The corrected fact row is written by the same delegate seeking to exit the work, and this branch *closes a Critical*, which is the strongest outcome in the loop. Before recording closed-by-disproof, re-run the brief's own named defect check yourself — you supplied it, so you have it — and confirm it does not exhibit the defect. Only then close. If it still exhibits the defect, the disproof is wrong: the finding is open, and this is an ordinary rejection to be re-decided. Note the asymmetry this closes: the `done` branch demands the defect check re-run and treats a bare claim as **not done**, so a disproof branch that closed on assertion alone would be the cheaper exit of the two, and it is the one that ends in a closed finding rather than a fixed one.
   ...
   **What a rejection costs — the termination argument, stated because "costs no round" alone does not terminate.** A rejection does not consume a **fix round**: the round budget bounds fix *attempts*, and a delegate that rejects a remedy has done more work than one that applies it. But an unbounded re-decide cycle has no exit — a delegate rejecting each successively re-decided remedy would loop forever on a budget nothing decrements. So the bound sits on the finding, not the round:
   ...
   - **A second rejection of a re-decided remedy converts the finding to Deferred-Known-Issue**, with both delegates' reasons as the rationale. It is closed for this run, recorded, reported, and carried to `TODO.md` — not silently dropped, and not looped on. Two independent rejections of two independently-decided remedies is strong evidence the finding needs a design conversation rather than another fix attempt, which is precisely what Deferred-Known-Issue is for.

   This keeps the promise that being right costs the delegate nothing, and still terminates in a bounded number of dispatches.
   ...
   **Receiving what it returns** — read the per-finding list, not the overall status, and route each row:
   ...
   A contract firing here — a `done` row without defect-gone evidence, a missing per-finding list, **or a missing `Fact corrections:` field** — is recorded as a Handoff issue, the same as Implement's. Recovering inline is correct; absorbing it silently hides the delegation-reliability signal.
...
2b. **Mutation gate (required before the commit in step 3).** A fix for a Critical/Important
   ...
   shown to catch anything, and an unrestored edit ships the defect. Fix a surviving mutant, or name it in the Handoff report with a reason. A clean
...
3. Stage + commit as `fix: review round <N>`. The fix-round commit subject is structurally meaningful (it drives `probe_state.py`'s round counter and the round-N-on-fix-diff scoping above). Before staging, verify git-state:
   ...
   **One carve-out, and it is not an exception to the guard's purpose but the case the guard cannot distinguish.** A round in which **every** Critical/Important finding was triaged Remedy-Rejected applied no edits *by design* — the delegate examined each proposed fix and returned reasons instead. There is correctly nothing to stage. Empty staging here is a **successful** round, not a failed one: skip the commit and push, do **not** mark `warn`, and **proceed to step 4** so the exit gate can see the open findings and re-enter. Without this, the headline success case of the whole three-status contract halts one step before the gate written to receive it, and a correct rejection is reported as a phase failure — the exact ledger distortion `remedy-rejected` exists to avoid.
   ...
   The guard's original target is untouched and is a different state: findings triaged **Applied**, so edits were promised, and nothing staged — that is a fix-applier reporting success without touching the file, and it still stops the round. **Discriminate by the triage, not by the empty output**, which looks identical in both cases: any Applied finding this round ⇒ empty staging is the failure; zero Applied findings ⇒ empty staging is expected.
...
4. **Exit gate — TWO counts, and both must be zero to exit clean.** *Triaged* and *closed* are different predicates, and one counter cannot serve for both.
   ...
   If **both** are 0 → status `ok`, exit loop. If open > 0 → **re-enter the loop** and re-attempt those findings with re-decided remedies, per the per-finding bound in step 2 ("What a rejection costs"): the first re-attempt is free and does **not** decrement the round budget, and a second rejection converts the finding to Deferred-Known-Issue rather than looping again. So re-entry here does not fall through to step 5's decrement — step 5 is for a round that made fix *attempts*. Suggestions still recorded for the optional `docs:` commit.
