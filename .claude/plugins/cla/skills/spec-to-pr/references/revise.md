...
## Dispatch mechanism differs by round (per `references/model-routing.md`)
...
Based on the diff content. Model + effort per `references/model-routing.md`; the **diff slice** column is the specialty-scoping rule (pass each agent only the hunks its specialty needs — the two bug-hunters get the full diff, the narrow agents get a filtered slice):
...
| a `.claude/plugins/cla/skills/*/SKILL.md` **frontmatter** (`description` / `argument-hint`) changed, OR a **new** skill/SKILL.md created, OR a new command under `.claude/commands/` created (rare for an app change — only when the change deliberately touches repo automation) | + `plugin-dev:skill-reviewer` | sonnet | medium | the SKILL.md file. **Trigger is frontmatter/new-skill-scoped, NOT "any SKILL.md byte touched"** — a body/section prose edit that leaves the frontmatter unchanged does not alter triggering behavior and should NOT pull this agent in. |
...
Dispatch via `Agent` (not Workflow), each agent one model tier down (`opus→sonnet`, `sonnet→haiku`; `haiku` stays `haiku`) — **EXCEPT `code-reviewer` and `silent-failure-hunter`, which stay `opus` per the never-demoted rule above**. Scope to the diff added by the previous fix commit. Inline the scoped diff into each agent's prompt — do NOT make agents re-read it. Capture the previous fix commit's sha **before** triggering the next round:
```
...
2. **Triage every Critical and Important finding into one of two buckets:**
   ...
   **Fix-delegate default (thin-orchestrator, per `references/runtime-rules.md`).** When the round's fix-set is large — past the existing sized-trigger (> ~15 subtasks-equivalent OR > ~8 files) — delegate the mechanical EDIT APPLICATION to the existing generic coding `Agent(model: sonnet)` (the same escape-hatch delegate Implement uses; there is NO separate "fix-applier" agent), with its evidence-based `done`/`blocked` terminal contract. **Delegation covers edit application ONLY — the orchestrator retains its own post-fix re-verification (INT-CAP/INT-SYC/SIR-TEST below); that discipline is never delegated.** Below the trigger, apply inline.

   **Subtle-implementation-risk findings need a proving test to count as Applied (SIR-TEST).** When a Revise finding is the "implementer followed the letter of a prior fix but missed its spirit" class — the pre-implementation Review's `review-change/references/checklist.md` names this class and mandates a proving-test *task* for it, but the Implement delegate can still ship an incomplete version, which is exactly why it resurfaces here — the fix is not fully Applied until a **dedicated regression test that would fail on the letter-but-not-spirit implementation** exists and passes. A prose/code edit that corrects the current instance without a test guarding the invariant leaves the defect one refactor away from returning. (Concrete precedents in this repo — a "reuse an existing helper" fix re-implemented with the wrong fallback value; a "gate this control like its true sibling" fix shipped with only half the gate tested — are in `references/project-context.md` "Incident history".) For a SIR finding, "Applied" = the correcting edit **plus** the proving test, both landed and passing.
