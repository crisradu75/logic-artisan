...
**Why `--specs` and not just the change.** `openspec validate <change> --strict` validates the CHANGE. Nothing here validated the **live spec set**, and this step is the one that rewrites it — so a materialization that lands a structurally broken `openspec/specs/<cap>/spec.md` was committed, pushed and merged with every check green. It needs no database and no build and takes seconds, which is why it belongs at every write site rather than at one.

**Read the output, not only the exit code.** Three outcomes, and the exit code alone collapses them into two wrong pairs:

- **`✗ spec/<cap>` in the output** → the live set is broken. **Halt and surface via `AskUserQuestion`**; do not commit. The per-requirement `[ERROR]` lines name the defect in words.
- **Non-zero with no `✗` line** (`is not recognized`, `command not found`, `unknown option`) → **the check could not run.** This is a tooling fault — a missing binary, a CLI too old for `--specs` — and attributing it to this change's specs sends the reader to the wrong subsystem entirely. Surface it as a tooling fault. `probe_state.py` keeps this distinction for the same reason and states it: reporting a tool fault as a content fault "points the reader at PATH for a problem PATH has nothing to do with."
- **`No items found to validate.`** → **not a pass.** Exit 0, and nothing was checked. A repo with no `openspec/specs/`, or a capability whose `spec.md` the materialization failed to write, both land here — and the two-sided assertion below cannot catch the second, because it checks *staging*, not existence. Confirm the `Totals:` line names at least one item. Where the repo does not use OpenSpec at all, say so once and skip the step rather than recording a vacuous success.

Verified against openspec **1.5.0**, which is where `--specs` and the "appears outside the main `## Requirements` section" diagnostic were measured; an older CLI may lack the flag, which is the second bullet's case.

**The check is wider than this commit.** `--specs` validates every live spec, not only the capabilities this change materializes — so in a chain it can fail on breakage an earlier change introduced. Read the `✗ spec/<cap>` line before assuming this materialization caused it.

The failure it catches is silent by construction and lands far from its cause — measured in a six-change chain in a repo consuming this plugin, and reported here rather than re-derived: two live specs each ended up with a **duplicated `## Requirements` heading**, the second of which closes the section, so every requirement in both became invisible to `validate`, `list` and `archive`. Both merged broken. It surfaced one whole change later, as an aborted archive. The rule and its worked shape live in `references/archive-preflight.md` check (a); this block is where it executes.
