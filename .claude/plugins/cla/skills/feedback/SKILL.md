---
name: feedback
description: "Interactive feedback capture: takes the user's notes one at a time until they say they're done, asks a clarifying question only when a note is genuinely ambiguous, and does a light READ-ONLY repo grounding pass per note (locating the relevant file/component/symbol and a probable root cause where clear, labeled as a hypothesis — never editing code). On done, consolidates into a dated markdown doc under cla.io/feedback/ grouped by area/screen with severity tags, merging near-duplicate notes only after the user confirms each merge. Output is triage, not decisions — it hands off to /cla:shape-decision. Triggers on /cla:feedback or natural language like 'capture my feedback notes', 'take these notes one at a time', 'consolidate my app feedback into a doc'."
