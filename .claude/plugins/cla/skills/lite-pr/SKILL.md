...
**Session-attribution lines go in the same block as the trailers, with no blank line between them.** `Co-Authored-By:` and `Claude-Session:` sit directly under the last `Measured-by:` line, inside one `-m`. Git parses trailers from the last paragraph of the message only. A blank line before the attribution puts every `Measured-by:` line outside the trailer block, so `git log --pretty='%(trailers:key=Measured-by,valueonly=true,unfold=true)'` returns nothing and the provenance hook records a count of zero for a commit that carries real trailers.
...
No pause before this runs — continuous by design (see Autonomy below). Use `commit-push-pr`'s own branch-naming and commit-message conventions as-is; lite-pr does not add any naming logic on top. The `Measured-by:` trailers are message *content* rather than a naming convention, so they are not an exception to that: hand them to `commit-push-pr` as part of the commit message it writes. Hand it the session-attribution lines in that same block too — a blank line between them and the trailers costs every `Measured-by:` line, because git reads trailers from the last paragraph only.
