...
- **Never end a turn with nothing in flight.** The rule above forbids *asking permission* to continue. This failure asks nothing: the orchestrator announces the next candidate, ends the turn believing it is continuing, and the chain does not pause — it **stops**, silently, until a human notices. The distinction that matters is not "did I ask a question" but **"is there a pending event that will re-invoke this session?"** A backgrounded `Agent`, `Workflow`, or `Bash(run_in_background: true)` is safe, because its completion notification wakes the session. An idle turn has no wake signal at all. **The mechanical form, because the judgement version has lost in practice: status text and the next tool call go in the SAME message.** If you have no tool call to pair the text with, the candidate is not over — start the next one instead of narrating the last. Ending a turn is legitimate in exactly three cases: a backgrounded dispatch is genuinely in flight, the chain is complete, or the chain is genuinely blocked on a decision already surfaced to the user. Nothing else — **announcing the next step is not a mechanism**, and "Is there a tool call in this message?" needs no judgement. Where a seam genuinely has nothing to dispatch, arm a watchdog (`Bash(command: "sleep 1500; echo WATCHDOG", run_in_background: true)`) before ending the message and re-arm it whenever it fires while work remains — a last resort that bounds the stall to one interval, never a substitute for the primary rule. Measured on `/cla:multi-pr`, which shares this loop shape: a six-change chain lost ~4.5 hours with 4 of 6 changes unstarted.
