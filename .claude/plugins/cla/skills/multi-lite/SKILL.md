...
- **`<base-branch>` means THIS repo's default branch, resolved — never assumed.** See `.claude/plugins/cla/skills/spec-to-pr/references/base-branch-resolution.md` (shared verbatim with `multi-pr`/`multi-spec`) for the resolution rule every command below that names `<base-branch>` depends on.
