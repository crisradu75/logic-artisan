...
- **Runs from the primary clone on `master` by default — that is what keeps its base management correct.** `multi-lite` does many real commits/pushes/PRs (and, for dependencies, merges) back-to-back, so — exactly like `/cla:multi-pr` — it operates on `master` in the primary clone and syncs between candidates with `git checkout master` then `git pull` (two separate commands). **A linked worktree cannot check out `master`** (git refuses — the primary clone already holds it), so the master-checkout steps below are valid only from the primary clone; do not "start this in a worktree." If another live session contends for the primary clone (a `guard-worktree-isolation.py` block mid-chain), pivot **reactively** to a dedicated worktree the way `/cla:multi-pr` does — `git fetch origin master` then `git worktree add .claude/worktrees/<id> -b <branch> origin/master` (two separate commands), basing each candidate off `origin/master` rather than checking out `master` — and do **not** reach for the hook's `ALLOW_SHARED_CLONE_MUTATION=1` escape hatch. In that reactive-worktree case the run may not end on `master`, so the run-log commit (see `references/phase4-and-log.md`) is best-effort and may be skipped.
- **Never run `git add -A`.** Same rule as `/cla:lite-pr`/`/cla:spec-to-pr`, inherited transitively through every `Skill(cla:lite-pr, ...)` call, and equally binding on any direct git command (merges, checkouts, syncs) `multi-lite` itself runs. Path-scope every stage per `.claude/plugins/cla/skills/spec-to-pr/references/bash-discipline.md`.
- **Verify `git_state` before every commit `multi-lite` itself makes, and verify each push landed.** Run `python3 .claude/plugins/cla/skills/spec-to-pr/scripts/git_state.py [--expect-branch <name>]` before the enforcement-round commit (Phase 3 step 7) and the run-log commit (a non-zero exit halts and surfaces); after a push, confirm `git rev-parse HEAD` matches `git ls-remote origin <branch>` (compared in-context). The per-candidate `/cla:lite-pr` runs do their own git_state checks internally.
...
- **Run thin — keep raw material out of context.** `multi-lite` is an orchestrator (chains `/cla:lite-pr` across candidates, layering merge/quarantine/review-enforcement logic around it) — apply `.claude/plugins/cla/skills/spec-to-pr/references/runtime-rules.md`'s delegation, I/O-hygiene, batching, and terse-output disciplines throughout the run, not just at one trigger point.
...
- `.claude/plugins/cla/skills/lite-pr/SKILL.md` — the single-candidate workflow this skill chains; its Test-phase halt and Review scope define what "failed" means in Phase 3.
- `.claude/plugins/cla/skills/multi-pr/SKILL.md` — the `spec-to-pr` analogue; source of the merge-before-dependents, autonomy-override, and worktree-pivot patterns reused here.
- `.claude/plugins/cla/skills/multi-spec/SKILL.md` — the inline-reasoning-over-prose precedent for extraction/sequencing without a bespoke script.
