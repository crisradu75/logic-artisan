...
- `branch`, `pr_number`, `head_sha` — filled at step 6, the moment a PR opens. `head_sha` is updated whenever step 7 pushes a fix or a resume adopts a moved head.
...
- `status` — `merged`, `open` (plain or with a reason), `failed`, `failed-review`, `failed-merge` with a reason, or `blocked-by-upstream-failure`. Step 8a's shared-state derivation goes beside it under `merge-dependencies-only`.
