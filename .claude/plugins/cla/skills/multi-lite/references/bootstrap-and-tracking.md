...
- `status` — `merged`, `open` (plain or with a reason), `failed` with the failing check or a reason, `failed-review` with a reason, `failed-merge` with a reason, or `blocked-by-upstream-failure`. Notes step 8 records go beside it: the shared-state derivation under `merge-dependencies-only`, `gate skipped: no source-affecting paths`, and `behind base: merged tree not tested`.
