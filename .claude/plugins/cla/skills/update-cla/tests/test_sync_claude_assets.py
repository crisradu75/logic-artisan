...
#
# Two subtleties whose failure modes are SILENT, which is why they are detected
# rather than documented: a synced hook that no-ops without a per-repo overlay,
# and a partially-applied lockstep group whose importers then fail to load.
# The declaration is read from the SOURCE, so a source that learns a new
# requirement teaches every consumer on their next sync.
# --------------------------------------------------------------------------- #

_REQ_PATH = ".claude/plugins/cla/skills/update-cla/references/sync-requirements.json"


def _requirements_doc(**kwargs) -> str:
    return json.dumps(kwargs)


def test_companion_overlay_flagged_when_the_overlay_is_absent(synthetic_repos):
    repos = synthetic_repos({
        "src": {
            ".claude/plugins/cla/hooks/warn-x.py": "new\n",
            _REQ_PATH: _requirements_doc(companion_overlays=[{
                "asset": ".claude/plugins/cla/hooks/warn-x.py",
                "requires": ".claude/plugins/cla/hooks/x.local.md",
                "note": "no-ops without it",
            }]),
        },
        "dst": {".claude/plugins/cla/hooks/warn-x.py": "old\n"},
    })
    result = _load("discover").discover(repos[0], repos[1])
    reqs = [r for r in result.requirements if r.kind == "companion_overlay"]
    assert len(reqs) == 1
    assert reqs[0].detail == ".claude/plugins/cla/hooks/x.local.md"
    assert "no-ops" in reqs[0].note


def test_companion_overlay_not_flagged_when_the_overlay_already_exists(synthetic_repos):
    repos = synthetic_repos({
        "src": {
            ".claude/plugins/cla/hooks/warn-x.py": "new\n",
            _REQ_PATH: _requirements_doc(companion_overlays=[{
        ...
        "dst": {
            ".claude/plugins/cla/hooks/warn-x.py": "old\n",
            ".claude/plugins/cla/hooks/x.local.md": "---\nk: v\n---\n",
        },
    })
    result = _load("discover").discover(repos[0], repos[1])
    assert [r for r in result.requirements if r.kind == "companion_overlay"] == []


def test_companion_overlay_not_flagged_when_its_asset_is_not_being_written(synthetic_repos):
    """Out of scope (unchanged, or filtered out) must not raise a warning —
    a notice that fires on every unrelated sync stops being read."""
    repos = synthetic_repos({
        "src": {
            ".claude/plugins/cla/hooks/warn-x.py": "same\n",
            ".claude/plugins/cla/skills/other.md": "new\n",
            _REQ_PATH: _requirements_doc(companion_overlays=[{
                "asset": ".claude/plugins/cla/hooks/warn-x.py",
                "requires": ".claude/plugins/cla/hooks/x.local.md",
            }]),
        ...
        "dst": {
            ".claude/plugins/cla/hooks/warn-x.py": "same\n",
            ".claude/plugins/cla/skills/other.md": "old\n",
        },
    })
    result = _load("discover").discover(repos[0], repos[1])
...
def test_partial_lockstep_group_is_flagged(synthetic_repos):
    """Only `_dispatch_lib` diverges here; its importer does not. Applying one
    without the other is the ImportError case."""
    repos = synthetic_repos({
        "src": {
            ".claude/plugins/cla/hooks/_dispatch_lib.py": "new\n",
            ".claude/plugins/cla/hooks/importer.py": "same\n",
            _REQ_PATH: _requirements_doc(lockstep_groups=[{
                "name": "shared helpers",
                "assets": [
                    ".claude/plugins/cla/hooks/_dispatch_lib.py",
                    ".claude/plugins/cla/hooks/importer.py",
                ],
                "note": "importer needs the lib",
            }]),
        },
        "dst": {
            ".claude/plugins/cla/hooks/_dispatch_lib.py": "old\n",
            ".claude/plugins/cla/hooks/importer.py": "same\n",
        },
    })
    result = _load("discover").discover(repos[0], repos[1])
    reqs = [r for r in result.requirements if r.kind == "lockstep_partial"]
    assert len(reqs) == 1
    assert reqs[0].asset_path == "shared helpers"
    assert ".claude/plugins/cla/hooks/importer.py" in reqs[0].detail


def test_a_whole_lockstep_group_moving_together_is_not_flagged(synthetic_repos):
    repos = synthetic_repos({
        "src": {
            ".claude/plugins/cla/hooks/_dispatch_lib.py": "new\n",
            ".claude/plugins/cla/hooks/importer.py": "new\n",
            _REQ_PATH: _requirements_doc(lockstep_groups=[{
                "name": "shared helpers",
                "assets": [
                    ".claude/plugins/cla/hooks/_dispatch_lib.py",
        ...
        "dst": {
            ".claude/plugins/cla/hooks/_dispatch_lib.py": "old\n",
            ".claude/plugins/cla/hooks/importer.py": "old\n",
        },
    })
    result = _load("discover").discover(repos[0], repos[1])
    assert [r for r in result.requirements if r.kind == "lockstep_partial"] == []


def test_an_untouched_lockstep_group_is_not_flagged(synthetic_repos):
    repos = synthetic_repos({
        "src": {
            ".claude/plugins/cla/skills/unrelated.md": "new\n",
            _REQ_PATH: _requirements_doc(lockstep_groups=[{
                "name": "shared helpers",
                "assets": [
                    ".claude/plugins/cla/hooks/_dispatch_lib.py",
        ...
        "dst": {".claude/plugins/cla/skills/unrelated.md": "old\n"},
    })
    result = _load("discover").discover(repos[0], repos[1])
    assert result.requirements == []


def test_a_malformed_requirements_file_warns_and_does_not_halt(synthetic_repos, capsys):
    """Absent-or-broken must not fail the sync, but must not be silent either:
    silence here is indistinguishable from 'nothing to warn about'."""
    repos = synthetic_repos({
        "src": {
            ".claude/plugins/cla/skills/a.md": "new\n",
            _REQ_PATH: "{not valid json",
        },
        "dst": {".claude/plugins/cla/skills/a.md": "old\n"},
    })
    result = _load("discover").discover(repos[0], repos[1])
    # `sync-requirements.json` lives under SCAN_DIRS and so is itself a synced
    # asset — deliberately, that is how a consumer receives the declaration —
    # hence assert on the specific file rather than a total count.
    assert ".claude/plugins/cla/skills/a.md" in {f.asset_path for f in result.files}, (
        "the sync itself still works"
    )
    assert result.requirements == []
    assert "sync-requirements" in capsys.readouterr().err


def test_no_requirements_file_at_all_is_silent(synthetic_repos, capsys):
    repos = synthetic_repos({
        "src": {".claude/plugins/cla/skills/a.md": "new\n"},
        "dst": {".claude/plugins/cla/skills/a.md": "old\n"},
    })
    result = _load("discover").discover(repos[0], repos[1])
    assert result.requirements == []
    assert "sync-requirements" not in capsys.readouterr().err


def test_the_real_repo_declaration_parses_and_names_existing_assets():
    """The shipped declaration must stay valid and must not name a path that no
    longer exists — a stale entry silently stops warning."""
    root = Path(__file__).resolve().parents[6]
    doc = json.loads((root / _REQ_PATH).read_text(encoding="utf-8"))
    named = [e["asset"] for e in doc.get("companion_overlays", [])]
    named += [a for g in doc.get("lockstep_groups", []) for a in g["assets"]]
    missing = [p for p in named if not (root / p).is_file()]
    assert missing == [], f"declaration names assets that do not exist: {missing}"
