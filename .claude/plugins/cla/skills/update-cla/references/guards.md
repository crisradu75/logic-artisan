...
- **No reverse propagation of CONTENT.** This skill pulls *to* the local repo only; no run ever writes into the source repo. What it does propagate backwards is **prose**: Phase 2.5 records a local improvement that belongs in the portable core as a proposal in `cla-upstream.md` at the destination repo root (see `references/upstream-proposals.md`), and the source repo collects from those files when it chooses to. That is a pointer, not a patch — actually moving the code still means running the skill in the source repo with this repo as the source, or porting the change by hand. The gap this closes is discovery: before it, a defect found during a sync was known only to the session that found it, so the same one was diagnosed independently in several repos.
