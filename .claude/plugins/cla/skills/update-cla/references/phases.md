...
1. **Read inputs:**
   ...
   - **The file's `status`** — since `cla-sync-provenance`, this now carries the 3-way classification, not just `divergent`/`new`. Act on it:
     ...
     A file may additionally carry **`kept_local_previously: true`** — a prior sync recorded a `keep_local` decision for it. That is evidence, not an instruction: someone already weighed this file and chose local. Re-deriving the same conclusion from scratch every run is how a deliberate divergence gets talked out of existence by the one run that does not think it through.
     - `adapted` — **neither side has moved since the last sync.** Local and source differ only because the adoption was adapted, which is the normal state of an adapted file. There is nothing upstream to pull: write an entry with `"keep_local": true` and no `adapted_content`. Do NOT "re-adopt source" here — that is the silent revert this label was added to prevent (before it existed, this exact case reported `source-advanced`, whose guidance says to adopt freely).
     - `source-advanced` — local is untouched since the last sync AND source has genuinely moved on. Adopt source, **re-applying the local adaptation** rather than taking raw source verbatim — for an adapted file the raw content is not what belongs here, and copying it over is the other half of the same silent revert.
     - `local-advanced` — source hasn't moved since the last sync, but local has; source would be a **regression** — keep local, don't overwrite it with the (older, in effect) source content unless source's newer diff still carries something genuinely new to cherry-pick. **Still write an entry for it**, with `"keep_local": true` and no `adapted_content`: every discovered file must be accounted for in `adaptations.json`, or `apply` reports it under `NOT ADAPTED` and exits 1. Omitting the entry is indistinguishable from dropping the file by mistake, which is the likeliest error in a 150-entry list.
