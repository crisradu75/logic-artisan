...
- `pr_number` / `final_branch_head` — the shipped PR and the branch-head sha at the point this record was logged, for attributing a ledger line to a concrete tree state (mirrors `multi-pr`'s `final_master_commit` field, scoped to this batch's own branch instead of `<base-branch>` since this skill never merges).
