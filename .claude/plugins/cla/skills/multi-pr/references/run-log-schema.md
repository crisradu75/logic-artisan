...
- `total_output_chars` (optional) — derive by summing `per_change[].output_chars` plus any chain-level orchestrator report text (e.g. Phase 4's cleanup summary), mirroring how `total_actual_min` sums `per_change[].approx_total_min`. A verbosity proxy for a future `multi-pr-retro`, NOT a full token-spend measure — it covers only printed report text.
- `final_master_commit` — the `<base-branch>` HEAD sha after the last merge + the chain-log commit's parent (i.e. the tip the whole chain produced), for attributing a ledger line to a concrete tree state.
