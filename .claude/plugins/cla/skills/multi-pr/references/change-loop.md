...
4a. **Record the obligations this change creates for later changes in the chain.** Runs once per change, after step 4 and before step 5/5-alt, so it sees the change as it FINALLY is.
   ...
   **Derive from what the change became, not from what it proposed.** The proposal predates the change's own Review and step-4 fix rounds, and those rounds are exactly where these obligations get created — a chain that reads the batch as proposed is reading a snapshot that went stale the moment any prerequisite's review round touched it. Two sources, both cheap and already in hand:
   ...
   - **Zero obligations is a real answer and is written down as `none`** beside this change's name, under the same heading. Without it a fresh-session resume cannot tell "this change owed nothing" from "nobody looked", and the second is the state the failure this step exists for actually occurred in.

   Done when the running notes' `## Carried obligations` section names this change, either with ≥1 row or with `none`. The rows are read back at step 2 of every later change that owes one, and nowhere else.
