...
3. **Read the Handoff report.** Two failure tiers, handled differently:
   ...
   **Tier B — content findings (fix, don't halt).** The Revise phase found Critical/Important issues and either applied them, deferred them, **or had a proposed remedy rejected by its fix delegate and left the finding open**; Test needed retries; a review round exhausted its cap with residue. **The third outcome is the one that hides**: a rejected-but-open finding is a live Critical, yet it reaches the report through a `warn` that looks like any other, so the phase glyph does not distinguish it. Read the Handoff report's "Rejected remedies, still open" section, not the glyph.

**A non-empty one blocks the merge under EVERY policy, not only the strict one.** Step 4's enforcement is strict-policy-only, so naming the section only there would fix the strict path and leave the default path merging over the finding — the original hole, under the policy most runs use. The looser policy was defined as accepting *Deferred-Known-Issue and Suggestion residue*: findings someone consciously chose to carry. A rejected-open finding is neither — nobody accepted it; a delegate examined the proposed fix and found it wrong, and the defect is real and unfixed. So under any policy: **do not run step 5's merge for this change.** Resolve it (re-decide the remedy and fix it, or get an explicit decision to accept it, which converts it to a Deferred-Known-Issue with a rationale), or leave the PR open and report it in the batch report. Never merge past it on the grounds that the policy is loose. This is normal `/cla:spec-to-pr` operation — proceed to step 4.
...
4. **No-unresolved-issues enforcement (only under the strict Phase 1 policy).** If the Handoff report's "Deferred Known Issues", **"Rejected remedies, still open"**, or "Deferred to TODO.md" sections are non-empty — under the full-severity default, this includes Suggestion-level residue, not just Critical/Important — don't accept that as final:
