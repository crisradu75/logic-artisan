...
5-alt. **Stack (only under the "stacked" policy — replaces step 5; steps 6-7 still run for this change).** No merge happens from the point this policy is in force. Once the change is genuinely done (same bar as step 5):
   ...
   - **Landing (the user's, at the end — Phase 4 hands over the checklist).** Parents first, and the merge STRATEGY matters: land each PR with `gh pr merge <#> --merge --delete-branch` — a **merge commit, not a squash**. Squash-merging a stacked parent rewrites its commits, so the child's retargeted PR re-shows the parent's entire diff and the child's own merge then conflicts wherever it touched the same region — measured on a throwaway 3-deep stack, not assumed. With a merge commit the parent's commits stay reachable from `<base-branch>`, each child's diff collapses to its own work, and deleting the head branch retargets the child's PR automatically (retargeting fires on the head-branch DELETION after merge, not on the merge itself — keep `--delete-branch`). If the repo requires squash-merges: after each parent lands, rebase its child before merging it — `git rebase --onto origin/<base-branch> <parent-tip-sha> <child-branch>` then `git push --force-with-lease` — using the parent tip sha step 2 recorded.
