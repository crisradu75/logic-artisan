...
1. **Resume check.** Before invoking `/cla:spec-to-pr`, check whether this change is already done: `openspec/changes/<name>/` no longer exists (archived) AND no open PR exists for `<branch>` (`gh pr list --head <branch> --state open`). If both hold, treat as already complete — mark its task done and skip to the next change. This makes a re-invoked `/cla:multi-pr` after an interruption resume correctly instead of re-running finished work.
