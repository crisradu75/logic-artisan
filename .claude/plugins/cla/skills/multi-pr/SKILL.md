...
- **An unresolved prerequisite is never guessed past either.** Phase 1a's sweep resolves every *change* a proposal names as a prerequisite — not just every doc — against `<base-branch>`, remembering that archive directories carry a `<YYYY-MM-DD>-` prefix and that an archived change with an open PR has NOT landed. A prerequisite whose code is on no merged branch (typically one existing only as an open, proposal-only PR) is its own explicit question before Phase 2, same standing as the cycle. Skipping this is a quiet failure: the chain looks healthy for hours until a dependent reads code nobody wrote.
