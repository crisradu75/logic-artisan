...
- **An unresolved prerequisite is never guessed past either.** Phase 1a's sweep asks, of every *change* a proposal names as a prerequisite — not just every doc — where that change's code actually lives. A prerequisite whose code exists **nowhere** (typically one existing only as an open, proposal-only PR) is its own explicit question before Phase 2, same standing as the cycle. One whose code is on an open PR is NOT a gate — that is the stacked parent, parented with `--pr-base`. Skipping the sweep is a quiet failure: the chain looks healthy for hours until a dependent reads code nobody wrote.
