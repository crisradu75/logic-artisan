...
- **An unresolved prerequisite is never guessed past either.** Phase 1a's sweep asks, of every *change* a proposal names as a prerequisite — not just every doc — where that change's code actually lives. Two conditions make it its own explicit question before Phase 2, same standing as the cycle: the code exists **nowhere** and no in-scope change in this run will write it, or it exists **only in a proposal-only PR** (specs and tasks, no code — `--pr-base` cannot supply what was never written). An open PR that *does* carry code is NOT a gate; that is the stacked parent, parented with `--pr-base`. Skipping the sweep is a quiet failure: the chain looks healthy for hours until a dependent reads code nobody wrote.
...
- Does not guess past a dependency cycle, an unresolved prerequisite, or an ambiguous merge decision — those are genuinely the user's calls, surfaced once, up front.

## References

- `references/discover-and-gate.md` — Phase 1's full mechanics: discovering and ordering the changes, the unmerged-dependency sweep over both cited docs and cited changes, the pre-flight-gate question wording (merge/no-unresolved-issues/caps/infra policies, plus the two structural questions), the explicit-autonomy and bounded-window-autonomy modes, infra self-remediation, and the chain-time-estimate recipe (mandatory-read from the Phase 1 stub)
