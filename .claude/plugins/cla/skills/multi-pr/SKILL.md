...
- **A prerequisite whose code exists nowhere is never guessed past either.** Phase 1a's sweep covers every *change* a proposal names as a prerequisite, not just every doc. One that no landed change, no in-scope change, and no open PR will supply is its own explicit question before Phase 2, same standing as the cycle. Skipping it is a quiet failure: the chain looks healthy for hours until a dependent reads code nobody wrote.
