...
0f–0i. **Domain-specific high-yield checks** — i18n key-parity + load-bearing daypart keys (0f), CSS-variable/styling convention (0g), mock-dataset shape/keying feasibility (0h), and operator seed-data/sample-file coverage (0i). These are repo-specific and as high-yield as 0a–0e for this repo — run them from the project overlay `references/project-context.md` ("Domain-specific high-yield checks").

0j. **Real-data/scale grounding check** — For any change that parses/decodes/bulk-loads an external data file (a CSV/JSON dump, a public dataset) or bulk-writes to a DB table carrying a maintained index: does the design's evidence of correctness rest only on small synthetic fixtures, or has an actual sample of the real file been inspected (even a truncated head, or a downstream repo's own real-file-sample-inspector script if one exists) for its real delimiter/encoding/column names/quoting convention? Two concrete failure classes to name explicitly, confirmed real in a downstream repo's own bulk-import work: (a) decoding a whole file into one JS string can exceed V8's ~536.8M-character string-length cap — flag a "read the whole file, decode as one string" design against any file that could plausibly exceed ~500MB; (b) a bulk `INSERT ... ON CONFLICT DO UPDATE` against a table with a maintained GIN index can exhaust Postgres's `max_locks_per_transaction` regardless of per-statement batch size — the index's internal "pending list" accumulates lock pressure ACROSS statements, not per-statement, so shrinking the batch size alone does not reliably fix it; the real fix is dropping/rebuilding the index around the bulk operation. A design whose only evidence is "the fixture test passes" has validated neither class.
...
**Empirical-verification fidelity (when a finding claims RUNTIME or DB semantics).** Most checks above are static (grep a symbol, read a file). Some findings instead assert *behavior* — "this single `UPDATE` can raise `23505` depending on row order," "this async path races that one," "this RPC returns a null-composite row." When you resolve such a finding by *running something* (a scratch query, a throwaway test), the harness MUST structurally mirror the real object: the SAME column names, the SAME constraint shape, and — critically — a genuinely non-unique grouping column when the real constraint is a partial/grouped unique index (multiple rows able to share the value the index is built on), never a simplified stand-in that is accidentally already unique (a primary key, a surrogate id). **A "verified" claim built on a structurally-wrong harness is worse than an unverified one — it carries false confidence into the next decision.** This is not hypothetical — see `references/project-context.md` ("Incident history") for a real case in this repo where a scratch-harness mismatch let a genuine Critical get wrongly disproved. **When a runtime/DB-semantics Critical is hard to verify faithfully in a scratch harness, the safest resolution is often to defer adjudication to the change's own implementation test — which is structurally faithful by construction — rather than to a hand-built scratch check.** (Also captured in user memory `feedback-postgres-empirical-verification-must-mirror-real-shape`.)

**Cost offload for large changes (default — thin-orchestrator discipline).** On a **large** change (per the Step-3 size gate — pre-compute `a`/`b`/`c`/`d` before this step to know, including the complexity-concentration override), the *mechanical* portion of checks 0a–0h (grep a symbol, read a reference/config file, confirm a file/line claim) **defaults to** a dispatch to the read-only `fact-gatherer` agent (haiku) rather than being run inline: hand it the list of artifact claims, and it returns the context-brief rows as a **structured pass/fail table** (each row resolving to verbatim evidence or an explicit NOT-FOUND, per the grounding contract), so the raw greps/reads stay out of the orchestrator's context. **You still adjudicate every ✗ row yourself** — the agent gathers facts, it does not decide whether a failed claim matters. Judgment may keep the mechanical checks inline for a borderline-small change (a dispatch costs more than the checks save on a truly small change), and small changes stay fully inline. This is the `cla-plugin` thin-orchestrator discipline (`.claude/plugins/cla/skills/spec-to-pr/references/runtime-rules.md`); it is defined here because the checklist is the shared source of truth, so the default **applies to BOTH** the `/cla:spec-to-pr` Review phase AND the standalone `/cla:review-change` path — an intended, shared behavior, not a spec-to-pr-private optimization. Routing rationale: `.claude/plugins/cla/skills/spec-to-pr/references/model-routing.md`.
...
**Injection is mandatory, not optional (Decision C).** Every `<inject: ...>` placeholder below MUST be replaced with the actual repo-fact content from `references/project-context.md` before the prompt is dispatched — the orchestrator reads the overlay (already done in Step 2) and pastes the relevant facts directly into the prompt text at dispatch time. A dispatched agent never loads the skill or resolves `references/project-context.md` itself, so a placeholder left un-filled, or replaced with a bare "see references/project-context.md" pointer, leaves that agent reviewing blind — strictly worse than embedding the facts. The check *structure* below (what to verify, in what order) is the portable part; the injected content is what makes each check concrete for this repo.

### Agent 1: Design Reviewer  (`model: opus`)

> You are a senior engineer reviewing an OpenSpec change for this repo. **Affected area:** <the app/package(s) Step 2 identified>. <inject: this repo's architecture + one-directional data-flow summary for the affected area, from `references/project-context.md` ("Repo context") — for the plain workspace member list/roles, pull from `cla.io/project-facts.md`'s "Workspace shape" instead>. Be terse — one line per finding, no preamble.
>
...
> **Relevant repo-convention doc content:** <inject: the repo's own architecture/convention doc(s) for the affected area, per `references/project-context.md`>
>
> Check:
...
> 2. Feasibility, completeness (missing edge cases — <inject: this repo's own domain-specific edge-case examples per affected area, from `references/project-context.md`>), unstated risks, scope, architecture fit (<inject: this repo's own architecture-boundary rules per affected area, from `references/project-context.md`>).
> 3. **Allocation-math integrity** (only if the change touches this repo's core calculation engine) — does it preserve the documented formulas? <inject: the exact formulas this repo's engine must preserve, from `references/project-context.md`>. Any silent formula change without an explicit behavior-change goal is Critical — and must update the load-bearing spec/doc files this repo names for that formula together (per `cla.io/project-facts.md`'s "Allocation-formula lockstep doc set", falling back to `references/project-context.md` if absent).
> 4. **i18n discipline** — Is all new user-facing text routed through this repo's translation-key mechanism, with the key added to every language file of the correct i18n layer? <inject: this repo's i18n-layer names + any load-bearing key convention (e.g. keys-not-display-strings), from `references/project-context.md`>.
> 5. **Convention fit** — <inject: this repo's styling-token, fixed-ordering, simulated-latency, and test-coverage conventions per affected area, from `references/project-context.md`>.
>
...
> 2. Spec testability (every SHALL has at least one WHEN/THEN scenario), conflicts with main specs, codebase pattern adherence (<inject: this repo's own architectural/authorization/i18n/styling/typechecking conventions, from `references/project-context.md`>).
